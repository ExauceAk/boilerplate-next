...
export default function ScrollDataTable<TData, TValue>({
  columns,
  data,
  tableClassName,
  headerClassName,
  ...
  return (
    <div className={cn(" rounded-xl  border border-primary-neutral-200  bg-white shadow-ta-xs", className)}>
      ...
      <Table className={cn(" rounded-md", tableClassName)}>
        <div className="sidebar max-h-72 w-full overflow-auto">
            ...
            {table.getHeaderGroups().map((headerGroup) => (
              ...
              >
                {headerGroup.headers.map((header) => (
                  <TableHead
                    key={header.id}
                    className={`${header.id === "select" ? "w-28" : header.id === "actions" && "w-44"} sticky top-0 z-10 w-full bg-white`}
                  >
